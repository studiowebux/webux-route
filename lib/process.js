...
function processAction(action, actions, route, router, log) {
  return new Promise((resolve, reject) => {
    try {
      log.debug("\x1b[33m", "webux-route - Process Action", "\x1b[0m");
      let URL = route + actions;
      URL = sanitizeURL(URL);
      log.info(
        "\x1b[33m",
        `webux-route - ${action.method.toLowerCase()} ${URL.toLowerCase()}`,
        "\x1b[0m"
      );
      routeType(router, action, URL);
      log.debug("\x1b[33m", "webux-route - Process Action Done", "\x1b[0m");
      return resolve();
    } catch (e) {
      throw e;
    }
...
async function processActions(actions, route, routes, router, log) {
  try {
    log.debug("\x1b[33m", "webux-route - Process Actions", "\x1b[0m");
    ...
    log.debug("\x1b[33m", "webux-route - Process Actions Done", "\x1b[0m");
  } catch (e) {
    throw e;
  }
}
...
async function processResources(route, routes, router, log) {
  try {
    log.debug("\x1b[33m", "webux-route - Process Resources", "\x1b[0m");
    ...
    log.debug("\x1b[33m", "webux-route - Process Resources Done", "\x1b[0m");
  } catch (e) {
    throw e;
  }
}
...
function processRoutes(routes, router, log) {
  return new Promise(async (resolve, reject) => {
    try {
      log.debug("\x1b[33m", "webux-route - Process routes", "\x1b[0m");
      for (const route of Object.keys(routes)) {
        if (typeof routes[route].resources === "object") {
          ...
          log.debug(
            "\x1b[31m",
            "webux-route - The resources for '" + route + "' isn't an object",
            "\x1b[0m"
          );
        }
      }
      log.debug("\x1b[33m", "webux-route - Processing Routes Done", "\x1b[0m");
      return resolve();
    } catch (e) {
      throw e;
    }
